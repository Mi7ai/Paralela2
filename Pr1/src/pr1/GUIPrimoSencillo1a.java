    ...
    btnComienzaCalculo.addActionListener( new ActionListener() {
        public void actionPerformed( ActionEvent e ) {
          if( txfNumero.getText().trim().length() == 0 ) {
            ...
            try {
            //5.2  Modifica la aplicacion para que cada numero que se quiera examinar sea evaluado por una
            //     hebra distinta creada ad hoc. 	
              long numero = Long.parseLong( txfNumero.getText().trim() );
              MiHebra4 t4 = new MiHebra4(numero);
              t4.start();
